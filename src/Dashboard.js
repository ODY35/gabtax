...
export default function Dashboard({ user, initialData, isReadOnly, t, lang, logo, adminSettings, setAdminSettings, allUsersData, setAllUsersData, onHelp, isLoading }) {
  ...
  const handleAddRevenue = (e) => {
    ...
    const taxes = calculateTaxes(currentMonthRevenue);
    const newData = {
      id: Date.now(),
  ...
  const handlePrefill = () => {
    const newData = generateSimulatedRevenueData(selectedIndustry);
  ...
  const annualTaxes = calculateTaxes(annualTotal);
    ...
    <main className="dashboard-content">
      ...
      {user.role === 'admin' && (
        <section className="admin-section glass-panel">
          ...
          <div className="input-field" style={{display:'flex', flexDirection:'column', gap:'10px', background:'transparent', border:'none'}}>
      ...
      {/* User Data Download (PDF Only for Basic Users) */}
      {user.role !== 'admin' && (
        <section className="glass-panel">
      ...
      <section className="stats-section glass-panel">
        ...
        <div className="stats-display">
          ...
          ) : user.role !== 'admin' && view === 'annual' ? (
            <div className="annual-summary">
              {/* Graphic Representation (Chart) */}
              <div className="summary-card" style={{flex:2}}>
                <h3>{t.revenueVsTax}</h3>
                <div className="chart-container">
                  {(() => {
                    const maxVal = revenueData.length > 0 ? Math.max(...revenueData.map(i => i.revenue)) : 0;
                    return revenueData.map(d => {
                      const hRev = maxVal > 0 ? (d.revenue / maxVal) * 100 : 0;
                      const hTax = maxVal > 0 ? (d.totalTax / maxVal) * 100 : 0;
                      return (
                        <div key={d.id} className="chart-bar-group">
                          <div className="chart-bar bar-revenue" style={{height: `${hRev}%`}} title={`Rev: ${d.revenue}`}></div>
                          <div className="chart-bar bar-tax" style={{height: `${hTax}%`}} title={`Tax: ${d.totalTax}`}></div>
                          <span className="chart-label">{t.months[d.monthIndex]}</span>
                        </div>
                      );
                    });
                  })()}
            ...
            <div className="annual-summary">
              ...
              <div className="summary-card">
                ...
                <ul className="tax-breakdown">
                  <li>TVA (18%): {annualTaxes.tva.toLocaleString()}</li>
                  <li>IS (30%): {annualTaxes.is.toLocaleString()}</li>
                  <li>Patente: {annualTaxes.patente.toLocaleString()}</li>
                </ul>
              </div>
            </div>
          )}
